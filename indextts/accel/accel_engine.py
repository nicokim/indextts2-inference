...
class AccelInferenceEngine:
    ...
    def _capture_cuda_graphs(self, tts_mel_embedding=None, tts_text_pos_embedding=None):
        logger.debug("Capturing CUDA graphs for decode optimization...")
        ...
        logger.debug("CUDA graphs captured for batch sizes: %s", self.graph_bs)
    ...
    def generate(
        ...
        if self.use_cuda_graph and not self.graph_captured:
            logger.debug("use_cuda_graph=%s, graph_captured=%s", self.use_cuda_graph, self.graph_captured)
            self._capture_cuda_graphs(
                tts_mel_embedding=tts_mel_embedding,
                tts_text_pos_embedding=tts_text_pos_embedding,
            )
            self.graph_captured = True
            logger.debug("CUDA graph capture completed! graphs=%s", list(self.graphs.keys()))
